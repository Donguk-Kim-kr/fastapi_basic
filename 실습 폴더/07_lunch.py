...
app = FastAPI(
    title="점심 메뉴 추천 API",
    description="오늘 뭐 먹을지 고민될 때 사용하는 메뉴 추천 API 입니다.",
    version="1.0.0"
)
...
def random_menu():
    ...
    랜덤 메뉴 1개 반환
    GET /menus/random
    주의: 반드시 특정 id 메뉴 반환보다 위에 정의되어야 한다.
    """
    return random.choice(menus) # ramdom.choice() - 선택한다.
...
@app.post("/menus", status_code=status.HTTP_202_ACCEPTED)
