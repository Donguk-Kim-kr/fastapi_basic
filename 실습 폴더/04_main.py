...
@app.get("/")
def root_handler():
    # dict를 반환하면 FastAPI가 자동으로 JSON으로 변환해서 응답
...
# 요청 PUT
# http://localhost:8000/items/5?assignee=손오공
# Body
#     {"name":"피카츄", "price":20000}
...
# response_model에 정의된 필트만 클라이언트에 전달된다. --> 보안
class OrderResponse(BaseModel):
    order_id: int
    pickup: bool | None = None # 없으면 Null 반환
...
def get_order_handler(order_id: int, pickup: bool | None = None):
    return {
        "order_id": order_id,
        "pickup": pickup # bool 쿼리 파라미터 ?pickup= true / ?pickup = 1 / ?pickup = yes
    }