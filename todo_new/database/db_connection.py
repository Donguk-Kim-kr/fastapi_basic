...
# echo=True --> 실행되는 SQL을 터이널에 출력 (학습용, 실무에서는 False)
engine = create_engine(DATABASE_URL, echo=True)

# 세션 팩토리 - 호출할 때마다 새로운 DB 세션(작업 단위) 객체를 만들어준다.
SessionFactory = sessionmaker(
    autocommit=False, # commit()을 직접 호출해야 DB에 반영한다.
    autoflush=False, # commit 전에 자동으로 SQL을 미리 보내지 않는다.
    expire_on_commit=False, # commit 이후에도 객체 값을 메모리에서 계속 사용 가능하다.
    bind=engine # 어떤 엔진(DB)에 연결할 지 지정
)
...
def get_session():
    ...
    # 함수 안에 yield가 있으면, yield 시점까지 실행한 뒤, 그 값(session)을 라우터 함수의 매개변수로 전달한다.
    # -> 라우터 함수 처리가 끝나면, FastAPI가 yield 다음 줄 (finally)을 실행해서 세션을 자동으로 닫아준다.
    # --> "언제 세션을 열고 언제 닫을지"를 라우터마다 안 써도 된다.(편리하다.)
    try:
        yield session
    finally:
        session.close()