...
# --> BaseModel 을 상속하여 스키마 기반 데이터 구조를 정의한다.
...
user = User(id="3", name="유화", email="test@1111.com")
print(user)

user2 = User(id="a", name="손오공", email="test2@1111.com")
print(user2)

# 왜 pydantic를 사용하는가?
# --> 외부 입력(사용자 메세지, API 응답, DB 검색 결과 등)을 많이 다루는데,
#     잘못된 데이터가 들어올 수 있다.
#     예외(에러) 발생 시 안전하게 처리 가능하다.
# --> 실제 값이 규칙에 맞는지 확인하고 필요하면 변환까지 해주는 도구
# --> 검증기

# BaseModel ??
# --> pydantic의 기본 클래스
# --> 우리가 만드는 데이터 클래스(모델)의 뼈대(스키마)를 정의할 떄 사용한다.
