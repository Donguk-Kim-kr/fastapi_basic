# ==========================================================================
# SQLAlchemy_2.0_football/crud.py
...
def get_player(db: Session, player_id: int):
    """
    기본키로 선수 한 명을 조회한다. 없으면 None을 반환
...
def get_players(db: Session, skip: int = 0, limit: int =100,
    ...
    # .scalars() --> 각 행에서 첫 번째 컬럼(여기서는 Player객체 자체)만 꺼내서
...
def get_league(db: Session, league_id: int | None = None):
    ...
    의도치 않은 에러가 발생할 가능성이 높다. 미리 None를 걸러낸다.
    """
...
def get_teams(db: Session, skip: int = 0, limit: int = 100,
    ...
    selectinload(models.Team.team_players)
        Team을 다 가져온 뒤, "team_players를 위한 추가 SELECT 1번" 더 날려서 한 번에 채운다.
        (joinedload처럼 JOIN해서 행이 늘어나는 방식이 아니라, 별도의 IN 조회를 한 번 더 하는 방식)

    .joinedload(models.TeamPlayer.player)
        그 다음 단계(TeamPlayer -> Player)는 seletionload가 만든 두 번째 쿼리 안에서 JOIN으로 같이 가져온다.

    --> 1단계 selectinload(추가 쿼리 방식),
        2단계는 joinedload(같은 쿼리 안에서 JOIN)를 체이닝해서 총 두 번의 쿼리로 team + team_player + player 정보를 다 채워온다.    
    """
...
def _count(db: Session, model: type) -> int:
    ...
    함수 앞의 밑줄(_count) --> 이 파일 안에서만 쓰는 내부 도우미 함수라는 뜻의 관례 표시

    select(func.count()).select_from(model)
        SQL문 "SELECT COUNT(*) FROM 테이블명"과 같은 뜻

    db.scalar(...) --> 결과 1개 값만 꺼낸다.
    """
