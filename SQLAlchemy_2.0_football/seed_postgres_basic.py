...
#     따라서, 테스트를 실행하기 전에 이 파일을 한 번 실행해서 CSV 데이터를 PostgreSQL 테이블에 넣어두어야 한다.
#
#     실행할 때마다 기존 테이블을 drop_all()로 삭제하고 다시 만든다.
...
def read_rows(filename: str) -> list[dict[str, str]]:
    ...
    row["player_id"] 처럼 컬렴명으로 값을 꺼낼 수 있다.
    """
...
def seed() -> None:
    """
    PostgraSQL 테이블을 새로 만들고, CSV 데이터를 순서대로 적재한다.
    ...
    - team_player.player_id는 player.player_id를 참조한다.
        참조 당하는 부모 테이블을 먼저 넣고, 참조하는 자식 테이블을 나중에 넣어야 한다.
    """
    ...
    with Session(engine) as session:
        ...
        # 2. league 테이블 적재 - league는 team에서 참조하므로 team보다 먼저 넣어야 한다.
        leagues = [
            models.League(
        ...
        # 뒤에서 team, team_player 처럼 외래키를 가진 테이블을 넣을 때 참조할 부모행이 없다는 오류를 피하고 실행 순서 파악도 쉽다.
        session.flush()
