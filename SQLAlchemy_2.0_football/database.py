# ====================================================================
# SQLAlchemy_2.0_football/database.py
#
#     - SQLAlchemy 2.0 버전으로 코드 변경
#     - PostgreSQL 데이터베이스와 연결
#     - 다른 파일들 (models.py, crud.py, main.py)에서
#     공통으로 사용할 engine / SessionLocal / Base 세 가지를 만들어 둔다.
# ====================================================================
import os
...
#                  지난 시간에 쓰던 declarative_Base() 함수 호출 방식 대신,
#                  진짜 파이썬 클래스를 상속하는 방식으로 변경되었다.
#
# sessionmaker : "세션 공장"을 만들어주는 함수
#                이 공장에서 세션을 하나씩 찍어내서 (SesstionLocal()) DB와 대화한다.
#
# os.getenv("DATEBASE_URL", 기본값) : 환경변수 DATABASE_URL이 설정되어 있으면
#                                    그 값을 사용하고, 없으면 두 번째 인자(기본값) 사용
# ----------------------------------------------------------------------------------
...
# pool_pre_ping=True
#     - 풀에서 연결을 꺼내 쓰기 전에 "SELECT 1" 같은 가벼운 쿼리를 먼저 날려서 그 연결이
#       아직 살아있는지 확인한다.
...
#     항상 session.commit() / session.rollback() 으로 트랜잭션 경계를
#     개발자가 직접, 명시적으로 정한다.
# ----------------------------------------------------------------------------------
