# =======================================================================
# SQLAlchemy_2.0_football/models.py
#     - SQLAlchemy 2.0 버전으로 코드 변경
#
# SQLAlchemy 모델 정의 (ORM 매핑 클래스)
#     - DB 테이블 5개 (player, performance, league, team, team_player)를
...
#
#     - team_player처럼 "중간 연결 테이블에 추가 데이터가 있는 M:N 관계"를 다룰 때 사용
# ---------------------------------------------------------------------------------
from sqlalchemy.ext.associationproxy import AssociationProxy, association_proxy

# ---------------------------------------------------------------------------------
# Mapped[T]
#     - 이 속성을 파이썬 코드에서 어떤 타입(T)으로 다룰지 표시
# mapped_column(..)
...
class Player(Base):
    ...
    # Mapper[str | None] --> 값이 문자열이거나 없을 수도 있는 컬럼
    gsis_id: Mapped[str | None] = mapped_column(String) # NFL 공식 선수 id
    ...
    # ---------------------------------------------------------------------------------
    # N:M 관계 (선수 여러 명 : 팀 여러 개) - 다대다 -> Association Object 패턴
    #
    ...
    #     선수가 team_player에서 제거되거나, 자체가 삭제되면, 관련된 TeamPlayer(가입기록)
    #     행도 같이 정리된다. (고아 상태로 남은 team_player행이 안 생기게 된다)
    # ---------------------------------------------------------------------------------
