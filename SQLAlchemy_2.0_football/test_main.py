# =================================================================================
# SQLAlchemy_2.0_football/testmain.py
#
# pytest로 단위 테스트
...
#     네트워크 소켓을 열지 않고도 훨씬 빠르게 같은 결과를 확인할 수 있다.
#
# 왜 이 도구들을 쓰는가?
# 1) Swagger UI에서 하나하나 클릭하고, 응답 결과를 눈으로 읽고, 숫자를 세는 행위는
#       느리고 지치고 실수가 생길 수 있다. 이 확인 과정을 "코드로 자동화"한다.
# 2) 회귀(regression) 방지
#       오늘 만든 기능이 내일 다른 코드 수정 때문에 망가지는 것을 자동으로 잡아내기 위해
#       pytest 한 줄 치면 수 십개 테스트가 몇 초만에 실행된다. --> 증거로 확인
# 3) TestClient를 사용하는 이유는 "서버를 안켜도 되니까"
#       진짜 서버(uvicorn)를 켜려먼 포트를 열고, 네트워크 소켓을 열고,
#       실제로 통신할 준비를 해야한다. --> 느리고, 포트 충돌 같은 부수적인 문제도 생긴다.
...
def test_read_main():
    """루트 헬스 체크 엔드포인트가 정상 메세지를 반환하는지 확인"""
    response = client.get("/")

    assert response.status_code == 200 # 요청 성공

    # .json()을 호출하면 응답 본문 문자열을 파이썬 딕셔너리로 파싱해준다.
...
def test_read_players():
    ...
    limit=10000 처럼 실제 선수 수(1018명)보다 훨씬 큰 값을 줘서 페이지네이션 때문에 일부만 잘리지 않고 전체가 다 오는지 확인
    """
...
def test_read_players_by_name():
    ...
    assert response.status_code == 200
    assert len(response.json()) == 1 # 1명만 나와야 한다.
