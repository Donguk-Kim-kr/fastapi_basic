# =================================================================================
# SQLAlchemy_2.0_football/main.py
#
...
#         --> Swagger UI의 "Try it out" 화면에서 각 입력칸 옆에 뜬다.
#
#     4) operation_id / tags 추가
#         --> operation_id : 이 API를 가리키는 고유한 이름,
#             tags : Swagger UI에서 엔드포인트들을 그룹으로 묶어 보여주는 기준이다.
# =================================================================================
...
api_description ='''
이 API는 Sports World Central(SMC) 판타지 풋볼 API의 정보를 읽기 전용으로 제공합니다.
...
## 멤버십(Membership)
SMC 판타지 풋볼 리그 전체와 각 리그에 속한 팀에 대한 정보를 제공합니다. 
'''

app = FastAPI(
    description=api_description,
    title="Sports World Central(SMC) 판타지 풋볼 API",
    version="0.1.1",
)
...
@app.get("/",
        ...
        description='''이 엔드포인트를 사용해 API가 실행 중인지 확인 합니다.
        다른 호출을 하기 전에 먼저 이 엔드포인트를 확인하면
        API가 정상 작동하는지 알 수 있습니다.''',
        # response_description : 이 API를 호출하면 어떤 응답이 오는지 설명한다.
        response_description='''메시지가 포함된 JSON 레코드입니다.
        API가 실행 중이면 성공 메시지를 반환합니다.''',
        # operation_id : 이 엔드포인트를 가리키는 전역 고유 이름.
        # URL(/)과 메서드(GET) 조합만으로는 사람이 읽기 힘들 수 있다.
...
def read_players(skip: int = Query(0, description="API 호출 시작 부분에서 건너뛸 항목 수입니다."),
                limit: int = Query(100, description="건너뛴 레코드(행) 이후 반환할 레코드(행) 수 입니다."), 
                minimum_last_changed_date: date = Query(None, description="변경 기준 날짜입니다. 이 날짜 이후에 변경된 레코드만 반환합니다."),
...
def read_performances(skip: int = Query(0, description="API 호출 시작 부분에서 건너뛸 항목 수입니다."),
                    limit: int = Query(100, description="건너뛴 레코드(행) 이후 반환할 레코드(행) 수 입니다."), 
                    minimum_last_changed_date: date = Query(None, description="변경 기준 날짜입니다. 이 날짜 이후에 변경된 레코드만 반환합니다."),
                    db: Session = Depends(get_db)):
    """선수 성적 목록 조회"""
...
def read_leagues(skip: int = Query(0, description="API 호출 시작 부분에서 건너뛸 항목 수입니다."),
                limit: int = Query(100, description="건너뛴 레코드(행) 이후 반환할 레코드(행) 수 입니다."), 
                minimum_last_changed_date: date = Query(None, description="변경 기준 날짜입니다. 이 날짜 이후에 변경된 레코드만 반환합니다."),
...
def read_teams(skip: int = Query(0, description='API 호출 시작 부분에서 건너뛸 항목 수입니다.'),
                limit: int = Query(100, description='건너뛴 레코드 이후 반환할 레코드 수입니다.'), 
                minimum_last_changed_date: date = Query(None, description='변경 기준 날짜입니다. 이 날짜 이후에 변경된 레코드만 반환합니다.'),
                team_name: str = Query(None, description='반환할 팀 이름입니다. SWC 전체서 고유하지 않으나, 리그 내부에서는 고유합니다'),
                league_id: int = Query(None, description='반환할 팀이 속한 리그 ID입니다. SWC에서 고유합니다.'),
                db: Session = Depends(get_db)):
    """
    팀 목록을 조회한다.
    
    league_id 조건으로 특정 리그의 팀만 가져올 수 있다.
    예) GET /v0/teams/?league_id=5001 --> 5001번 리그 소속팀만 응답
    """
...
@app.get("/v0/counts/", response_model=schemas.Counts,
        ...
        description='''이 엔드포인트를 사용해 SWC 판타지 풋볼의 리그, 팀, 선수 수를 확인합니다.
        다른 조회 엔드포인트의 skip과 limit 매개변수와 함께 사용합니다.
