...
#         Delete : 삭제 DELETE(FastAPI) DELETE(SQL) --> ex) 회원탈퇴, 글 삭제
# pgAdmin --> database --> reviewdb 생성

from sqlalchemy.orm import Session
from pwdlib import PasswordHash
import models, schemas # 복습/ 안에 있는 파일들 models.py, schemas.py
...
def get_users(db: Session, skip: int = 0, limit: int = 100):
    ...
    # (페이지네이션 : 데이터를 한 번에 다 보여주지 않고 페이지 단위로 나눠서 보여주는 것)
    # .limit(limit): 최대 limit개만 반환
    # .all(): 조건에 맞는 전체 결과를 리스트로 반환
    return db.query(models.User).offset(skip).limit(limit).all()

def create_user(db: Session, user: schemas.UserCreate):
    # 평문 비밀번호(user.password)를 해싱(원래 값을 알 수 없는 암호화 문자열로 치환)하여 저장
