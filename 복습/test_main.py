...
def test_create_user(client):
    """유저 생성 -> 200 응답, 이메일 위치, 비밀번호 미노출, items 빈 리스트 확인"""
    ...
    assert "hashed_password" not in data # 해시 비밀번호도 없음

    # 처음 생성 시 아이템이 없으므로 빈 리스트
    assert data["items"] == []

def test_create_user_duplicate_email(client):
    """같은 이메일로 재가입 시도 -> 400 에러 + 에러 메세지 확인"""
    response = client.post(
        "/users/",
        json={
    ...
    # 이 테스트는 400이 나와야 PASSED -> "실패해야 통과"하는 케이스
...
def test_user_has_items(client):
    """유저 조회 시 items 배열에 아이템이 포함되어있는지 확인 (1:N 관계 검증)"""
    response = client.get("/users/1")
