...
#
# 커넥션(connection)이란?
# - 파이썬 프로그램과 PostgreSQL 서버 사이에 이어지는 연결 통로
# - 계속 SQL을 주고 받으려면 이 연결부터 먼저 잇는다.
...
# - 다 쓰고 나면(with 블록이 끝나면) 연결을 끊는 게 아니라 풀에 다시 반납해서, 다음 번 요청이 그 연결을 재사용할 수 있게 한다.
# ============================================================
...
def verify():
    ...
    with engine.connect() as conn:
        ...
        # --- 1. 전체 적재 건수 확인 ---
        # .scalar() --> 결과에서 첫 번째 행의 첫 번째 컬럼 값 하나만 뽑아온다.
        #               (값이 하나만 나오는 쿼리에 사용)
        total = conn.execute(text("SELECT COUNT(*) FROM subway_raw")).scalar()

        # --- 2. 필수 컬럼별 NULL 개수 확인 ---
        # FILTER (WHERE 조건) --> 하나의 SELECT 안에서 조건별로 COUNT를 따로 집계하는 PsostgreSQL의 문법
        ...
        # --- 4. 시작시 (시간) 범위 이탈 확인 ---
        # 시작시는 0 ~ 23시 범위 (24시간제 기준) - BETWEEN - AND 23
