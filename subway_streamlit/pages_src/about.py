"""
subway_streamlit\pages_src\about.py

작성일 : 26-08-06
작성자 : 김동욱
목적 : 다른 페이지에서 st.session_state에 저장해둔 값들을 여기서 그대로 읽어낼 수 있는지 확인
       --> "st.session_state가 페이지를 넘어가도 유지된다" 라는 점을 알아두자.
"""
...
if "favorite_stations" in st.session_state and st.session_state.favorite_stations:
    ...
    st.write("즐겨찾는 역: 아직 설정되지 않았습니다. '역별 탐색' 화면에서 추가해보세요.")
...
if "last_search_result" in st.session_state:
    ...
    st.write("아직 '검색(폼) 화면'에서 검색을 실행하지 않았습니다.")