"""
subway_streamlit\pages_src\search.py
...
목적 : 폼 안에 입력 위젯들(역 이름, 승하구분, 기간, 시작시 범위, 요일) 조건들을 조작할 때마다
       ...
       만약에 데이터가 많을 때 폼 없이 위젯을 조작하고 필터링을 다시 하면 체감 속도가 느려진다.
       --> st.form을 사용한다.
"""
import pandas as pd
...
with st.form("subway_search_form"):
    ...
    # 버튼 - 폼 안에서 버튼을 st.form_submit_button
...
if submitted:
    ...
    if keyword:
        # filtered["역명"].str.contains --> 문자열의 일부만도 검색하려고(포함여부)
    ...
    # 슬라이더는 (최솟값, 최댓값) 튜플로 반환 -> 시작시가 그 범위 안에 있는 행만 남김
    filtered = filtered[
    ...
    if len(filtered) > 500:
        st.caption(f"표에는 상위 500건만 표시됩니다. 전체 {len(filtered):,}건은 아래 버튼으로 다운로드 하세요.")
