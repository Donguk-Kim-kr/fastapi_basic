...
if file is not None:
    ...
    st.dataframe(df.head()) # 5줄.

    st.divider

    # 2. 보고 싶은 컬럼 선택 (multiselect)
    ...
    # 숫자형 컬럼만 필터 대상으로 선택가능하도록 추출
    numeric_cols = df.select_dtypes(include="number").columns.tolist()

    # 3. 점수 범위 필터링 (slider) - 숫자형 컬럼이 있을 때만
    if numeric_cols:
        target_col = st.selectbox("범위를 필터링할 컬럼을 선택하세요.", numeric_cols)
        ...
        # 4. 조건에 맞는 행만 필터링
        filtered_df = df[
            (df[target_col] >= low) & (df[target_col <= max])
        ]
        ...
        st.warning("범위 필터링에 사용할 숫자형 컬럼이 없습니다.")
