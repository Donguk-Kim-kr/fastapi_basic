...
#         이번 예제에서는 Python 코드가 데이터를 직접 계산하지 않고,
#         PostgreSQL에게 SQL을 실행시켜 DB엔진이 집계하도록 위임한다.
#         데이터가 매우 클 때에는 이렇게 DB(또는 Spark) 쪽에서 처리하는 것이
#         Python으로 한 줄씩 읽어 계산하는 것보다 훨씬 빠르고 메모리도 효율적이다.
# ===========================================================================
from database import bus_engine, subway_engine, check_required_tables, execute_sql

def create_subway_hourly_summary() -> None:
    """
    지하철 원본 데이터(subway_raw)를 "역번호+역명+승하차+시작시" 기준으로
    그룹핑하여 시간대별 이용 인원 집계 테이블을 만든다.
    
    AVG("인원수")::numeric
        --> AVG 결과값을 numeric타입으로 반환해라.
            PostgreSQL 문법
            CAST(AVG("인원수) AS numeric)가 표준 SQL 문법
    """
    execute_sql(
        subway_engine,
        """
...
def create_bus_area_summary() -> None:
    """
    버스 정류소 원본 데이터(sub_stop)를 "시군구(구/군)" 기준으로 집계하여
    지역별 정류소 개수 및 좌표 누락 개수를 담은 테이블을 만든다.

    SQL 설명 :
    - COALESCE("시군구", '미분류') : "시군구" 같이 NULL이면 "미분류" 문자열로 대체
...
# (다른 파일에서 impor batch_processor로 불러올 때는 실행되지 않는다. 모듈 재사용을 위한 관용구)
