...
def check_required_tables() -> None:
    """
    배치/실시간/이벤트 처리를 시작하기 전에 반드시 존재해야 하는
    원본 테이블(subway_raw, bus_stop)이 실제로 존재하고 데이터가 들어있는지 점검하는 함수.
    ...
    for engine, table_name, hint in checks:
        ...
        if count == 0:
            # 테이블은 존재하지만 적재된 행이 0건인 경우 (저장시스템 실습 미완료 가능성)
...
def execute_sql(engine, sql: str, params: dict | None = None) -> None:
    """
    여러 문장으로 이루어진 SQL 스크립트(세미 콜론으로 구분)를 한번에 실행한다.

    동작 방식 : 
        1) engine.begin() : 트랜잭션을 시작하고, with 블록이 정상 종료되면
                            자동으로 COMMIT, 예외가 발생하면 자동으로 ROLLBACK
        2) sql.sqlit(";") : SQL 문자열을 세미콜론 기준으로 나누어 여러 statement로 분리
