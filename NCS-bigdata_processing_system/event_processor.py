...
#           2) BUS_COORDINATE_OUT_OF_RANGE : 버스 정류소 좌표가 대구 범위를 벗어난 [이상 데이터] 이벤트
#           3) BUS_LOCATION_GROUP_INVALID : 버스 정류소 "시군구" 값이 허용 목록에 없는 이벤트
#
#     - 실무의 CEB(Complex Event Processing) 엔진(Esper, Flink CEP 등)은 이런 규칙을
...
#         실제 파이썬 같은 별도의 딕셔너리로 DB 드라이버에 전달하는 방식
#         임계값, 날짜, 이름처럼 "데이터 값"을 바인딩 한다.
#         테이블명, 컬럼명, ASC/DESC 같은 "SQL 구조"는 보통 값 파라미터로 바인딩할 수 있다.
#         --> 허용 목록(화이트리스트)으로 검증
#
#     3) 멱등성(idempotency)
...
def detect_bus_coordinate_events() -> None:
    ...
    2) BUS_LOCATION_GROUP_INVALID
           "시군구" 값이 NULL이거나, BUS_LOCATION_GROUPS에 없는 값이면 이상 데이터로 간주
    """
    ...
    execute_sql(
        ...
        SELECT
            ...
            CONCAT('location_group=', COALESCE("시군구", 'NULL')),
            :detected_at
        FROM bus_stop
        WHERE "시군구" IS NULL OR "시군구" NOT IN ({allowed_values});
        """,
        {"detected_at": datetime.now()}
    )
    print("[event] 버스 좌표/시군구 이벤트 탐지 완료")
...
def run_event_processing(subway_threshold: int = 100000) -> None:
    ...
    detect_bus_coordinate_events() # 버스 좌표/시군구 이벤트 탐지
