...
from datebase.db_connection import engine, SesstionFactory
from datebase.orm import Base
...
def get_todos_handler():
    ...
    try: # stmt : SQL문을 의미하는 statement 의 약자.
        stmt = select(Todo) # 데이터를 조회하는 쿼리 객체(아직 데이터베이스에는 접근하지 않았다.)
        # session.execute(stmt) : 쿼리 객체를 실체 데이터베이스에 전달해서 실행
...
def get_todo_handler(todo_id: int):
    ...
    try:
        ...
        todo = session.execute(stmt).scalars().first() # 첫 번째 결과 1개만
        if todo:
            return todo
        # 없으면 404 에러
        raise HTTPException(
...
def update_todo_handler(todo_id: int, body: TodoUpdateRequest):
    ...
    try:
        ...
        if todo:
            ...
            #     title만 보내면 title만 수정, is_done만 보내면 in_done만 수정)
